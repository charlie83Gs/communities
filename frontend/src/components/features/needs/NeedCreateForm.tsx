...
import { useCreateNeedMutation } from '@/hooks/queries/useNeeds';
import type { CreateNeedDto, NeedPriority, NeedRecurrence } from '@/types/needs.types';
...
import { needCreateFormDict } from '@/components/features/needs/NeedCreateForm.i18n';

interface NeedCreateFormProps {
  communityId: string;
  canManageItems?: boolean;
  onCreated?: () => void;
}

export const NeedCreateForm: Component<NeedCreateFormProps> = (props) => {
  const t = makeTranslator(needCreateFormDict, 'needCreateForm');
  ...
  const createMutation = useCreateNeedMutation();
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div class="flex gap-4">
              <label class="flex items-center gap-2 cursor-pointer">
                ...
                <span class="text-sm text-stone-900 dark:text-stone-100">
                  {t('need')} <span class="text-xs text-stone-600 dark:text-stone-400">({t('needHelp')})</span>
                </span>
              </label>
              <label class="flex items-center gap-2 cursor-pointer">
                <input
                ...
                />
                <span class="text-sm text-stone-900 dark:text-stone-100">
                  {t('want')} <span class="text-xs text-stone-600 dark:text-stone-400">({t('wantHelp')})</span>
                </span>
              </label>
            </div>
          </div>
          ...
          <div class="md:col-span-2 border-t border-stone-200 dark:border-stone-700 pt-4 mt-2">
            <div class="flex items-center gap-2 mb-3">
              ...
              />
              <label for="recurring-need" class="text-sm font-medium text-stone-900 dark:text-stone-100">
                {t('recurringLabel')}
              </label>
            </div>
            ...
            <Show when={isRecurring()}>
              <div class="pl-6 space-y-3">
                <p class="text-xs text-stone-600 dark:text-stone-400 mb-3">{t('recurringHelp')}</p>
