...
export class CouncilController {
  ...
  /**
   * @swagger
   * /api/v1/communities/{communityId}/councils/{councilId}/inventory:
   *   get:
   *     summary: Get council inventory
   *     tags: [Councils]
   *     security:
   *       - bearerAuth: []
   *     parameters:
  ...
  async getInventory(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const { councilId } = req.params;
      const result = await councilService.getInventory(councilId, userId);
  ...
  /**
   * @swagger
   * /api/v1/communities/{communityId}/councils/{councilId}/transactions:
   *   get:
   *     summary: Get council transaction history
   *     tags: [Councils]
   *     security:
   *       - bearerAuth: []
   *     parameters:
   ...
   *     responses:
   *       200:
   *         description: Council transactions
   *       401:
   *         description: Unauthorized
   *       403:
   *         description: Forbidden
  ...
  async getTransactions(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const result = await councilService.getTransactions(councilId, userId, {
        page: page ? Number(page) : 1,
        limit: limit ? Number(limit) : 20,
      });
